...
import BetterCam  # Takes screenshot. Now using BetterCam for efficiency.
import configparser
...
def load():  # loading the settings, duh.
    global cam, center, screenshot, AIM_KEY, SWITCH_MODE_KEY, FOV_KEY_UP, FOV_KEY_DOWN, CAM_FOV, AIM_OFFSET_Y, AIM_OFFSET_X, AIM_SPEED_X, AIM_SPEED_Y, upper, lower, UPDATE_KEY, AIM_FOV, BINDMODE, COLOR, colorname, TRIGGERBOT, TRIGGERBOT_DELAY, SMOOTHENING, SMOOTH_FACTOR, TRIGGERBOT_DISTANCE
    ...
    try:
        ...
        cam = BetterCam.Camera()
        cam.open(0)  # Assuming the default camera is index 0
        screenshot = {
            "width": CAM_FOV,
            "height": CAM_FOV,
            "left": int((cam.width / 2) - (CAM_FOV / 2)),
            "top": int((cam.height / 2) - (CAM_FOV / 2))
        }
        center = CAM_FOV / 2
...
class trb0t:
    ...
    def process(self):  # process all images we're capturing
        if rbxfocused():
            try: 
                img = cam.grab(screenshot)  # grab screenshot using BetterCam
